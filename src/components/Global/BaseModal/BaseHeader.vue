...
<template>
    <div class="yl pa-4 d-flex align-center" block>
        <v-btn class="close-btn" color="black" @click="$_closeModal()" icon="mdi-close" />
        <v-icon color="white" size="x-large">{{ icon }}</v-icon>
        <div class="mr-2">
            <h4>{{ title }}</h4>
            <h6 :style="{ letterSpacing: sm ? '5px' : isMobile() ? '0px' : '10px' }">{{ subtitle }}</h6>
        </div>
    </div>
</template>
